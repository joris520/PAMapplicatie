...
<table class="dashboard" cellspacing="0" cellpadding="0" style="width:{$interfaceObject->getDisplayWidth()};">
    <tr>
        ...
        <th class="" colspan="{$keyColCount}" style="padding-top:10px;" title="{'TITLE_NUMBER_OF_TARGETS'|TXT_UCF}">{'NUMBER_OF_TARGETS'|TXT_UCF}</th>
    </tr>
    <tr>
        ...
        <th class="last" style="width:130px; padding-top:10px;">{'NO_TARGETS'|TXT_UCF}</th>
        <th class="seperator" style="width:20px; padding-top:10px;">&nbsp;</th>
        {foreach $keyIdValues as $keyIdValue}
            ...
            <th class="last{if $key == EmployeeTargetStatusValue::NO_STATUS_EXPIRED} alternate{/if}" style="padding-top:10px;">{$keyLabel}</th>
        {/foreach}
    </tr>
    ...
    <tr>
        ...
        {foreach $keyIdValues as $keyIdValue}
            {assign var=key value=$keyIdValue->getDatabaseId()}
            <td class="last{if $key == EmployeeTargetStatusValue::NO_STATUS_EXPIRED} alternate{/if}">&nbsp;</td>
        {/foreach}
    </tr>
    ...
    <tr style="text-align:center; font-weight:bold;" id="detail_dashboard_totals" onmouseover="activateThisRow(this);" onmouseout="deactivateThisRow(this);">
        ...
        {foreach $keyIdValues as $keyIdValue}
        {assign var=key value=$keyIdValue->getDatabaseId()}
        <td class="last icon-link {if $key == EmployeeTargetStatusValue::NO_STATUS_EXPIRED} alternate{/if}">
