...
function getMainNavButtons($should_restore_main_nav = 0)
{
    ...
    if (PermissionsService::isEditAllowed(PERMISSION_JOB_PROFILES_LIBRARY)) {
        $add_buttons .= InterfaceBuilder::IconButton('btn_benchmarking',
                                                    ...
                                                    'xajax_moduleFunction_benchmarkingOptions();',
                                                    ICON_SETTINGS,
                                                    btn_width_150);
    }
    if (PermissionsService::isAddAllowed(PERMISSION_JOB_PROFILES_LIBRARY)) {
        $add_buttons .= InterfaceBuilder::IconButton('btn_add_profile',
                                                    ...
                                                    'xajax_moduleFunction_addFunction();',
                                                    ICON_ADD,
                                                    btn_width_150);
    }
    return $add_buttons;
        ...
        while ($getf_row = @mysql_fetch_assoc($getf)) {
            ...
            if (PermissionsService::isEditAllowed(PERMISSION_JOB_PROFILES_LIBRARY)) {
                $buttons = InterfaceBuilder::IconButton('edit_function' . $function_id,
                                                       ...
                                                       'storeScrollPos();xajax_moduleFunction_editFunctionComptence(' . $function_id . ');',
                                                       ICON_EDIT,
                                                       NO_BUTTON_CLASS,
                                                       FORCE_USE_ICON);
            }
            if (PermissionsService::isEditAllowed(PERMISSION_JOB_PROFILES_LIBRARY)) {
                $buttons .= InterfaceBuilder::IconButton('edit_function' . $function_id,
                                                        ...
                                                        'storeScrollPos();xajax_moduleFunction_deleteFunction(' . $function_id . ');',
                                                        ICON_DELETE,
                                                        NO_BUTTON_CLASS,
                                                        FORCE_USE_ICON);
            }
            ...
            <tr>
                <td class="dashed_line ' . $function_class . '" style="vertical-align:top;">
                    <a href="" onclick="storeScrollPos();xajax_moduleFunction_displayFunctionCompetence(' . $function_id . ','. RESTORE_NAV_MENU . ');return false;">' . $function . '</a>
                </td>
...
function moduleFunction_displayFunctionCompetence($id_f, $should_restore_main_nav = 0) {
    $objResponse = new xajaxResponse();
    if (PamApplication::hasValidSession($objResponse) && PermissionsService::isViewAllowed(PERMISSION_JOB_PROFILES_LIBRARY)) {
        ...
        if (@mysql_num_rows($getfp) > 0) {
            $top_nav_print_btn = InterfaceBuilder::IconButton('genPrint',
                                                             ...
                                                             'xajax_moduleFunction_printJobProfile(' . $id_f . ');',
                                                             ICON_PRINT,
                                                             btn_width_150);
...
function getFunctionsSelectForCopy($select_message)
{
    ...
    if (@mysql_num_rows($get_f) > 0) {
        $html .= $select_message . '
        <select name="ID_F" id="id_f" onchange="xajax_moduleFunction_useExistingProfile(this.options[this.selectedIndex].value);return false;">
...
//ADD NEW FUNCTION
function moduleFunction_addFunction() {
    $objResponse = new xajaxResponse();
                        ...
                        <table border="0" cellspacing="0" cellpadding="1" width="99%">
                            ...
                            while ($ksp_row = @mysql_fetch_assoc($ksp_rec)) {
                                ...
                                if ($ksp_row[is_key] == '1') {
                                    ...
                                    //1-5 or N/Y
                                    if ($ksp_row[scale] == ScaleValue::SCALE_Y_N) {
                                        $click = 'onclick="xajax_moduleFunction_hideScaleDetails(' . $ksp_row[ID_KSP] . ', \'Y/N\');return false;"';
                                    } elseif ($ksp_row[scale] == ScaleValue::SCALE_1_5) {
                                        $click = 'onclick="xajax_moduleFunction_hideScaleDetails(' . $ksp_row[ID_KSP] . ', \'1-5\');return false;"';
                                    }
                                } else {
                                    $key_comp = SIGN_IS_NOT_KEY_COMP;
                                    $checked = '';
                                    //1-5 or N/Y
                                    if ($ksp_row[scale] == ScaleValue::SCALE_Y_N) {
                                        $click = 'onclick="xajax_moduleFunction_showScaleDetails(' . $ksp_row[ID_KSP] . ', \'Y/N\');return false;"';
                                    } elseif ($ksp_row[scale] == ScaleValue::SCALE_1_5) {
                                        $click = 'onclick="xajax_moduleFunction_showScaleDetails(' . $ksp_row[ID_KSP] . ', \'1-5\');return false;"';
                                    }
                                }
...
//        $get_dict = @mysql_fetch_assoc($competenceQuery);
        $valueObject_KSP = FunctionKnowledgeSkillPointValueObject::createValueObject($id_ksp);
...
//        $get_ks = @mysql_fetch_assoc($categoryQuery);
        $valueObject_KS = FunctionKnowledgeSkillValueObject::createValueObject($valueObject_KSP->ID_KS);
...
//        $get_c = @mysql_fetch_assoc($clusterQuery);
        $valueObject_KSC = FunctionKnowledgeSkillClusterValueObject::createValueObject($valueObject_KSP->ID_C);

        $get_category = empty($valueObject_KS->knowledge_skill) ? EMPTY_CLUSTER_LABEL : CategoryConverter::display($valueObject_KS->knowledge_skill);
        $get_cluster = empty($valueObject_KSC->cluster) ? EMPTY_CLUSTER_LABEL : $valueObject_KSC->cluster;
...
function moduleFunction_useExistingProfile($id_f)
{
    $objResponse = new xajaxResponse();
    if (PamApplication::hasValidSession($objResponse) && PermissionsService::isAddAllowed(PERMISSION_JOB_PROFILES_LIBRARY)) {
...
function functions_processSafeForm_addFunction($objResponse, $safeFormHandler)
{
    $hasError = true;
    if (PermissionsService::isAddAllowed(PERMISSION_JOB_PROFILES_LIBRARY)) {
...
//DELETE FUNCTION
// TODO: refactor
function moduleFunction_deleteFunction($id_f) {
    $objResponse = new xajaxResponse();
...
// TODO: refactor
function moduleFunction_editFunctionComptence($id_f) {
    $objResponse = new xajaxResponse();
        ...
        $getf_data .= '
                        ...
                        <table border="0" cellspacing="0" cellpading="0">
                            <tr>
                                ...
                                <td>
                                    <input type="button" value="' . TXT_BTN('CANCEL') . '" class="btn btn_width_80" onclick="xajax_moduleFunction_displayFunctionCompetence(' . $id_f . ', ' . RESTORE_NAV_MENU . ');return false;">
                                </td>
                            </tr>
                        </table>
                    </form>
...
function moduleFunction_showScaleDetails($ksp_id, $scale2) {
    $objResponse = new xajaxResponse();
    if (PamApplication::hasValidSession($objResponse) && PermissionsService::isViewAllowed(PERMISSION_JOB_PROFILES_LIBRARY)) {
        ...
        $objResponse->assign('com' . $ksp_id, 'innerHTML', '<input type="checkbox" name="com' . $ksp_id . '" value="' . $ksp_id . '" checked onclick="xajax_moduleFunction_hideScaleDetails(' . $ksp_id . ', \'' . $scale2 . '\');return false;">');
    }

    return $objResponse;
}

function moduleFunction_hideScaleDetails($ksp_id, $scale2) {
    $objResponse = new xajaxResponse();
    if (PamApplication::hasValidSession($objResponse) && PermissionsService::isViewAllowed(PERMISSION_JOB_PROFILES_LIBRARY)) {
        if ($scale2 == ScaleValue::SCALE_Y_N) {
        ...
        $objResponse->assign('click_scale' . $ksp_id, 'innerHTML', $scale);
        $objResponse->assign('click_weight' . $ksp_id, 'innerHTML', $weight_factor3);
        $objResponse->assign('com' . $ksp_id, 'innerHTML', '<input type="checkbox" name="' . $ksp_id . '" value="' . $ksp_id . '" onclick="xajax_moduleFunction_showScaleDetails(' . $ksp_id . ', \'' . $scale2 . '\');return false;">');
...
//EXECUTE ADD NEW FUNCTION
function functions_processSafeForm_editFunctionCompetence($objResponse, $safeFormHandler)
{
    $hasError = true;
    if (PermissionsService::isEditAllowed(PERMISSION_JOB_PROFILES_LIBRARY)) {
        ...
        if (!$hasError) {
            ...
            while ($ksp_row = @mysql_fetch_assoc($get_ksp)) {
                $ID_KSPT = $ksp_row['ID_KSP'];
                $com_id = $safeFormHandler->retrieveSafeValue('com'.$ID_KSPT);
                if ($ksp_row['ID_KSP'] == $com_id) {
...
function moduleFunction_printJobProfile($id_f)
{
    $objResponse = new xajaxResponse();
...
//        <form id="empPrintForm" method="POST" action="javascript:void(0);" onsubmit="empPrintProfileExecute();return false;">
        ...
        $print_btns = '<input type="button" id="btnEmpPrint" value="&laquo; ' . TXT_BTN('BACK') . '" class="btn btn_width_80" onclick="xajax_moduleFunction_displayFunctionCompetence(' . $id_f . ', ' . RESTORE_NAV_MENU . ');return false;">
                       <input type="button" id="btnEmpPrint" value="' . TXT_BTN('PRINT_PROFILE') . '" class="btn btn_width_80" onclick="xajax_moduleFunction_executePrintFunctions(xajax.getFormValues(\'empPrintForm\')); return false;">';
...
function moduleFunction_executePrintFunctions($fForm)
{
    $objResponse = new xajaxResponse();
