...
if (isset($user) && defined('CUSTOMER_ID') &&
    ModuleUtils::isValidLogin() && // korte check of we ingelogd zijn volgens pam_config
    (PermissionsService::isAddAllowed(PERMISSION_EMPLOYEE_ATTACHMENTS) || PermissionsService::isAddAllowed(PERMISSION_ASSESSMENT_PROCESS_MARK_EVALUATION_DONE))) {
...
//    global $smarty;
    ...
    list($upload_html, $frameHeight) = handle_upload_evaluation_attachment($xajax, $employee_id);
    // resize iframe truukje
    //echo '<script type="text/javascript">parent.document.getElementById("upload_evaluation_attachment").height=' . $frameHeight .'</script>';
}
echo $upload_html;
?>