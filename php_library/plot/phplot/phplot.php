...
/////////////////////////////////////////////
    ...
    function ProcessTextGD($draw_it, $font_number, $font_width, $font_height, $angle, $x, $y, $color, $text, $h_factor, $v_factor) {
    ...
    function ProcessTextTTF($draw_it, $font_file, $font_size, $angle, $x, $y, $color, $text, $h_factor, $v_factor) {
        ...
        for ($i = 2; $i < 8; $i += 2) {
            $x_b = $c[$i];
            if ($x_b < $bbox_ref_x)
                $bbox_ref_x = $x_b; elseif ($bbox_max_x < $x_b)
                $bbox_max_x = $x_b;
            $y_b = $c[$i + 1];
            if ($y_b < $bbox_ref_y)
                $bbox_ref_y = $y_b; elseif ($bbox_max_y < $y_b)
                $bbox_max_y = $y_b;
        }
...
/////////////////////////////////////////////
    ...
    function SetDataType($which_dt) {
        ...
        if ($which_dt == 'text-linear') {
            $which_dt = 'text-data';
        } elseif ($which_dt == 'linear-linear') {
            $which_dt = 'data-data';
        } elseif ($which_dt == 'linear-linear-error') {
            $which_dt = 'data-data-error';
        } elseif ($which_dt == 'text-data-pie') {
...
//////////////////////////////////////////////////////////
    ...
    function FindDataLimits() {
        ...
        for ($i = 0; $i < $this->num_data_rows; $i++) {
            ...
            if ($miny < $minminy)
                $minminy = $miny;   // global min
 if ($maxy > $maxmaxy)
                $maxmaxy = $maxy;   // global max
...
// function DrawXTicks
    ...
    function DrawDataLabel($which_font, $which_angle, $x_world, $y_world, $which_color, $which_text, $which_halign = 'center', $which_valign = 'bottom', $x_adjustment=0, $y_adjustment=0) {
